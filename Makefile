...
TOOLCHAIN ?= docker.io/golang:1.26.2-alpine
...
all: unit-tests kms-server image-kms-server lint
...
.PHONY: image-kms-server
image-kms-server:  ## Builds image for kms-server.
	@$(MAKE) registry-$@ IMAGE_NAME="kms-server"
