...
Конфигурация nginx разворачивается через Ansible-роль [`talos-kms`](https://gl.brooklyn.wf/devops/iac/-/tree/main/ansible/roles/talos-kms).
...
Секреты зашифрованы через ansible-vault и хранятся в [`devops/iac/ansible/roles/talos-kms`](https://gl.brooklyn.wf/devops/iac/-/tree/main/ansible/roles/talos-kms) (`vars/vault.yml`).
...
SSH-подключение к `kms.mgmt.brooklyn.wf` под пользователем `ansible` (sudo, NOPASSWD).
...
| `KMS_BASE_URL` | Базовый URL admin API, по умолчанию `https://kms.mgmt.brooklyn.wf` |
...
3. Ansible скачивает бинарник из Registry и деплоит на `kms.mgmt.brooklyn.wf`
